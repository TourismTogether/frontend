...
async function getRoutePath(
  ...
  try {
    ...
    console.warn('Error fetching route path:', error);
    return null;
  }
}
...
function createRouteMarkerIcon(routeNumber: number, color: string, isStart: boolean = false) {
...
export const RouteMap: React.FC<RouteMapProps> = ({
  ...
  const [routePaths, setRoutePaths] = useState<Map<string, [number, number][]>>(new Map());
  const [loadingPaths, setLoadingPaths] = useState(false);

  // Filter out routes with invalid coordinates
  const validRoutes = useMemo(() => {
    return routes.filter(isValidRoute).sort((a, b) => (a.index || 0) - (b.index || 0));
  }, [routes]);
  ...
  useEffect(() => {
    const fetchRoutePaths = async () => {
      ...
      // Fetch paths for each route
      await Promise.all(
        validRoutes.map(async (route) => {
          const routeKey = route.id || `${route.latStart}-${route.lngStart}-${route.latEnd}-${route.lngEnd}`;
          
          // Try to get actual road path from OSRM
    ...
    return (
      <div className="w-full rounded-lg overflow-hidden border border-gray-300 bg-gray-100 flex items-center justify-center" style={{ height }}>
        <div className="text-center p-8">
          ...
          <p className="text-gray-400 text-sm">Add routes with valid coordinates to see them on the map</p>
        </div>
      </div>
    );
  }

  // Create connected path for all routes
  const connectedPath = useMemo(() => {
    if (validRoutes.length === 0) return [];
    
    const path: [number, number][] = [];
    
    // Add first route's start point
    path.push([validRoutes[0].latStart, validRoutes[0].lngStart]);
    
    // Add each route's end point and connect to next route's start
    validRoutes.forEach((route, index) => {
      path.push([route.latEnd, route.lngEnd]);
      
      // Connect to next route if exists
      if (index < validRoutes.length - 1) {
        const nextRoute = validRoutes[index + 1];
        path.push([nextRoute.latStart, nextRoute.lngStart]);
      }
    });
  ...
  return (
    <div className="w-full rounded-lg overflow-hidden border-2 border-gray-200 shadow-lg" style={{ height }}>
      <MapContainer
        center={mapCenter}
        zoom={zoom}
        ...
        {validRoutes.map((route, index) => {
          ...
          return (
            <React.Fragment key={route.id || `route-${index}`}>
              ...
              {/* Start marker with route number */}
              <Marker
                position={[route.latStart, route.lngStart]}
                icon={index === 0 ? createSpecialMarkerIcon('start', routeNumber) : createRouteMarkerIcon(routeNumber, color, true)}
              >
                <Popup className="route-popup">
                  <div className="p-3 min-w-[200px]">
                    ...
                    <p className="text-sm text-gray-600 mb-2">{route.description || 'No description'}</p>
                    <div className="border-t pt-2 mt-2 space-y-1">
                      <p className="text-xs text-gray-500">
                        <span className="font-semibold">Start:</span> {route.latStart.toFixed(6)}, {route.lngStart.toFixed(6)}
                      </p>
                      <p className="text-xs text-gray-500">
                        <span className="font-semibold">End:</span> {route.latEnd.toFixed(6)}, {route.lngEnd.toFixed(6)}
                      </p>
                        ...
                        <div className="mt-2">
                          ...
                          <ul className="text-xs text-gray-600 space-y-0.5">
                            ...
                            {route.details.length > 3 && (
                              <li className="text-gray-400">+{route.details.length - 3} more</li>
                            )}
                          </ul>
                        </div>
                      )}
              ...
              <Marker
                ...
                icon={
                  index === validRoutes.length - 1 
                    ...
                    : createRouteMarkerIcon(routeNumber + 1, routeColors[(index + 1) % routeColors.length], false)
                }
              >
                <Popup className="route-popup">
                  <div className="p-3 min-w-[200px]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="font-bold text-base text-gray-900">
                        {index === validRoutes.length - 1 
                          ? `Final Destination: ${route.title || 'Route ' + routeNumber}`
                          : `Route ${routeNumber} End → Route ${routeNumber + 1} Start`
                        }
                      </p>
                    </div>
                    <p className="text-sm text-gray-600 mb-2">
                      {index === validRoutes.length - 1 
                        ...
                        : 'Connection point to next route'
                      }
                    </p>
                    <div className="border-t pt-2 mt-2">
                      <p className="text-xs text-gray-500">
                        <span className="font-semibold">Coordinates:</span> {route.latEnd.toFixed(6)}, {route.lngEnd.toFixed(6)}
                      </p>
                    </div>
                  </div>
                </Popup>
      ...
      {validRoutes.length > 0 && (
        <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-lg border border-gray-200 p-3 z-[1000] max-w-xs">
          <p className="text-xs font-bold text-gray-700 mb-2 uppercase tracking-wide">Route Legend</p>
          <div className="space-y-1.5 max-h-48 overflow-y-auto">
            {validRoutes.map((route, index) => {
              const routeNumber = (route.index || 0) + 1;
              const color = routeColors[index % routeColors.length];
              return (
                <div key={route.id || index} className="flex items-center gap-2 text-xs">
                  <div 
                    className="w-3 h-3 rounded-full flex-shrink-0" 
                    style={{ backgroundColor: color }}
                  ></div>
                  <span className="font-semibold text-gray-700">Route {routeNumber}:</span>
                  <span className="text-gray-600 truncate">{route.title || 'Untitled'}</span>
                </div>
              );
            })}
          </div>
