...
export default function Diaries() {
  ...
  const [filter, setFilter] = useState<string>("explore");
  ...
  async function handleDelete(id: any) {
    ...
    try {
      ...
      setAllDiaries((prev) =>
        prev ? prev.filter((d: any) => d.id !== id) : prev
      );
      setDiaries((prev) =>
        prev ? prev.filter((d: any) => d.id !== id) : prev
      );
      ...
      <div className="min-h-screen bg-background">
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* CARD */}
            {diaries &&
              diaries.map(function (diary: any) {
                return (
                  <div
                    key={diary.id}
                ...
                <div className="bg-white rounded-lg p-6 z-10 max-w-sm w-full shadow-lg">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 border bg-green-600 text-white rounded text-sm cursor-pointer"
                    >
                      Yes
                    </button>
                    <button
